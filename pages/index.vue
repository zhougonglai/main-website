...
section.w-full.flex.flex-col.pb-20
  ...
  dot-class="bg-white w-2 h-2 rounded-full hover:scale-125"
    ...
    carousel-item.flex.w-full(v-for="b in banner.filter(b => b.status)" :key="b.id")
      a.w-full(:href="b.name")
        img.object-center.object-cover.w-full.h-full(:src="b.url")
  ...
  .sc-link.flex.justify-center.py-20(v-if="link.length")
    .link-block.container.px-0.flex.flex-col.items-center(class="lg:px-20")
      h1.text-4xl.pl-10.w-full 快速链接
      ul.links.grid.gap-x-5.mt-10(class="grid-cols-1 lg:grid-cols-5 w-full lg:w-fit")
